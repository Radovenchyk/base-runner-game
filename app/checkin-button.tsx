...
export function CheckinButton() {
  ...
  // Чекаємо поки SDK готовий, тільки тоді підключаємо
  useEffect(() => {
    if (isReady && !isConnected && !tried && connectors.length > 0) {
      setTried(true);
      connect({ connector: connectors[0] });
    }
  }, [isReady, isConnected, connectors]);
  ...
  return (
    <div style={{ textAlign: "center" }} onClick={stop}>
      <button
        style={{ ...btn, background: isPending || isConfirming ? "#555" : "#0052ff" }}
